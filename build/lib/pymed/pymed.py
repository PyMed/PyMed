"""
 PyMed Module assisting to launch queries, get and subsequently process PubMed
 records.

 Authors: Denis A. Engemann, email: denis.engemann@gmail.com
          Danilo Bzdok, email: danilobzdok@gmail.com

 License: BSD (3-clause)

 Resources:

 PubMed search keys: http://www.nlm.nih.gov/bsd/mms/medlineelements.html
 Biopython Medline, Entrez http://www.bio-cloud.info/Biopython/en/ch8.html
...
class Records(list):
    ...
    def filter(self, func, inplace=True):
        """ Convenience method for returning a reduced set of records

        Parameters
        ----------
        records : list of pymed.PubmedRecord | filter expression
            A list of selected records or an expression that selects records.
    ...
    def insert(self, index, record):
        ...
        """
        if self.exclude_:
            raise RuntimeError('Indexes must be dropped before inserting'
                               'new records.')
        else:
...
def query_records(term, client, pubmed_fields='all', chunksize=50):
    ...
    while True:
        ...
        for rec in Medline.parse(handle):
            recs += [PubmedRecord(dict((k, v) for k, v in rec.items()
                     if match(k)))]
